...
var (
	...
	filterPort = flag.Int("port", 0, "指定仅输出该端口的结果，0表示不过滤")
)
...
func main() {
	...
	for _, ip := range ips {
		...
		go func(ip string) {
			defer func() {
				...
				percentage := float64(atomic.LoadInt64(&count)) / float64(total) * 100
				fmt.Printf("已完成: %d / %d (%.2f%%)\r", count, total, percentage)
	...
	if *speedTest > 0 {
		writer.Write([]string{"IP地址", "端口", "TLS", "数据中心", "地区", "国际代码", "国家", "城市", "网络延迟", "下载速度(MB/s)"})
	} else {
	...
	for _, res := range results {
		// 端口过滤
		if *filterPort != 0 && res.result.port != *filterPort {
			continue
		}
	...
	fmt.Printf("✅ 成功将结果写入文件 %s，耗时 %d 秒。\n", *outFile, int(time.Since(startTime).Seconds()))
}
...
func readIPsFromFile(filePath string) ([]string, error) {
    ...
    if strings.ToLower(filepath.Ext(filePath)) == ".csv" {
        ...
        for i, record := range records {
            ...
            ipAddr := strings.TrimSpace(record[1]) // 第2列 ip
            portStr := strings.TrimSpace(record[2]) // 第3列 port
            if ipAddr == "" || portStr == "" {
                continue
            }
            port, err := strconv.Atoi(portStr)
...
func getDownloadSpeed(ip string, port int) float64 {
    ...
    // 先发送HEAD请求获取文件大小和是否支持Range
    headReq, _ := http.NewRequest("HEAD", fullURL, nil)
    ...
    if err != nil || fileSize == 0 {
        ...
        contentLengthStr = getResp.Header.Get("Content-Length")
        fileSize, err = strconv.ParseInt(contentLengthStr, 10, 64)
        if err != nil || fileSize == 0 {
            fmt.Printf("IP %s 端口 %d 测速失败\n", ip, port)
            return 0
    ...
    // 并发下载
    chunkSize := fileSize / int64(*speedTest)
    if chunkSize < 1024*1024 { // 确保每个块至少1MB
        chunkSize = 1024 * 1024
        *speedTest = int(fileSize / chunkSize)
        if *speedTest == 0 {
            *speedTest = 1
        }
    }
    ...
    go func() {
        time.Sleep(10 * time.Second) // 最大测速时间10秒
        select {
        case <-done:
            return
        default:
